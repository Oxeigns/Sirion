...
from OxygenMusic.utils.database import get_banned_users, get_gbanned
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("OxygenMusic.plugins" + all_module)
    LOGGER("OxygenMusic.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Aviax.start()
    try:
        await Aviax.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    except NoActiveGroupCall:
        LOGGER("OxygenMusic").error(
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("OxygenMusic").info("Stopping Shruti Music Bot...")
