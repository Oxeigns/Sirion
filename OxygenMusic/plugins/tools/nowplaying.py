...
@app.on_message(filters.command(["nowplaying", "cnowplaying"]) & filters.group & ~BANNED_USERS)
@language
async def now_playing(client, message: Message, _):
    ...
    if "live_" in file_path or "index_" in file_path:
        ...
        if videoid == "telegram":
            image = config.TELEGRAM_VIDEO_URL if streamtype == "Video" else config.TELEGRAM_AUDIO_URL
